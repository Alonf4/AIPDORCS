...
export function text66_mouseIn(event) {
   $w('#arrow').expand();
}
export function text66_mouseOut(event) {
   $w('#arrow').collapse();
}
...
export function inputProjectID_click(event) {
   $w('#inputProjectID').value                 = $w('#text65').text;
...
let beams = 100; let columns = 50; let walls = 100; let slabs = 25;
...
export function calculate_click(event) {
	$w('#overallScore').value = 100 - ($w('#slider1').value + $w('#slider2').value + $w('#slider3').value + 
